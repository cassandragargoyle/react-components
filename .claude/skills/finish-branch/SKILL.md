...
`gh pr create --repo cassandragargoyle/react-components --base main`; put `Fixes #N` in
the body so the issue is linked.
...
3. **Close the GitHub issue** `#N` (== `NNN`) and update the link in its body to
   ...
   (for ❌ Closed / won't-fix, `--reason "not planned" --comment "<why>"`)
