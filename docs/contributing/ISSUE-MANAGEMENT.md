...
`#N` matches exactly one file `NNN`: `#1` ⇄ `001-*.md`, which the documents call
**INT-001**. The three names are the same issue.

GitHub numbers issues and pull requests from **one shared sequence**. A pull request opened
before the next issue consumes a number, so file numbers can have gaps; that is expected and
the gap is never filled.
...
Issues referenced from the source as `#100`, `#105` or "issue 125" predate this repository:
they are `portunix-vscode` issues, from which the components migrated.
...
1. **Take the number**, with a body you will replace in step 3:
   ...
   ```bash
   gh issue create --repo cassandragargoyle/react-components \
     --title "<title>" --body "<summary, see step 3>"
   ```

2. **Write the file** `docs/issues/NNN-slug.md` — see [The file](#the-file). Everything of
   substance goes here.

3. **Set the GitHub issue title and body** to `NNN: <title>` and the summary and the link,
   never the whole content:
   ...
   **Type:** enhancement · **Priority:** medium · **Status:** 📋 Open
   ...
   📄 Full issue: [`docs/issues/NNN-slug.md`](https://github.com/cassandragargoyle/react-components/blob/main/docs/issues/NNN-slug.md)
   ```

   ```bash
   gh issue edit N --repo cassandragargoyle/react-components \
     --title "NNN: <title>" --body-file body.md
   ```
...
The layout the assistant writes:
...
- **Created**: YYYY-MM-DD
- **Author**: Name
- **Target**: the component this lands in (`src/<Component>/`)
- **GitHub**: [#N](https://github.com/cassandragargoyle/react-components/issues/N)
- **Related**:
  - [INT-MMM — Title](MMM-slug.md) — why it matters here
...
3. **Close the GitHub issue** and repoint its link at the new path:
   ...
   ```bash
   gh issue close N --repo cassandragargoyle/react-components --reason completed
   ```

   For ❌ Closed, use `--reason "not planned"` and say why with `--comment`.
...
Priority is `critical`, `high`, `medium` or `low`, and it lives in the file's Metadata,
because the file has to be readable without GitHub.
